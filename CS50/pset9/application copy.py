...
def index():
    ...
    for soma in db.execute("SELECT value FROM users JOIN possession ON possession.user_id = users.id"):
        total += soma['value']

    return render_template("index.html", money = row[0]['cash'], total = "{:.2f}".format(total))
